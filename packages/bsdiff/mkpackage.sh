...
PKG_VERSION=$(curl -u "${GITHUB_USER_NAME}:${GITHUB_API_TOKEN}" -s "https://api.github.com/repos/chiyokolinux/packages/contents/packages/bsdiff/archives" | grep name | sed -E 's/^.*: ?\"[a-zA-Z]*\-([0-9\.\-]+)\.tar\.[gxb]z2?\",?/\1/' | tail -n1)
